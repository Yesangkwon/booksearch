...
const auth = getAuth();
//비밀번호로 로그인 처리 함수이다. - firebase auth가 제공하는 함수임.
//그래서 import해야 함. - module방식(import) <-> commonjs방식(require)
//login.ejs에서 입력받은 이메일과 비번을 각각 두 번째와 세 번째 파라미터로 넣어줌
//이 함수 요청으로 firebase에서 처리(시간지연발생-비동기)를 한다.
//- 성공하면 -> then이후가 진행됨(자동으로 진행됨-콜백)
//응답 결과를 호출해준 함수의 콜백 부분에 파라미터 자리인 userCredential담아줌
